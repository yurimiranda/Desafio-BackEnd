...
using MotorcycleRental.Api.Shared;
using MotorcycleRental.Host.Resources;
    ...
    public static IApplicationBuilder UseExceptionHandler(this IApplicationBuilder builder, Serilog.ILogger logger)
    {
        return builder.UseExceptionHandler(app =>
        {
            app.Run(async context =>
            {
                ...
                logger.Error(exceptionHandlerFeature?.Error, httpRequestIdentifierFeature?.TraceIdentifier, Messages.InternalServerError);

                context.Response.StatusCode = 500;
                string text = JsonSerializer.Serialize(new InternalError
                {
